...
- There are three steps to pre-process the data:
  ...
  - **Data processing** - We need to organize the data by formatting, cleaning and sampling
  - **Data transformation** - We need to transform data into a simple format for storage and future use such as a CSV, spreadsheet, or database file. 
...
- **Null values and missing data:**
  ...
  - Drop null or missing values or drop the entire column. 

- **Datatype:** All data has to be converted to numerical data type. 

- **Duplicates:** Remove duplicates as they may lead to errors.
...
- PCA reduces the number of dimensions by transforming a large set of variables into a smaller one that contains most of the information in the original, large set.
  ...
  3. These new components are new main dimensions of variations that contain most of the information in the original dataset.
...
- **3D graph and PCA visualization**
  ...
  - When hovering over the specific element the graph shows the label with Coin Name and its Algorithm. 
...
- **2D visualization and correlation between Total Coins Supply and Total Coins Mined**
  ...
  - When hovering over the specific element the graph shows the label with Coin Name and its Class.
...
- **Interactive Table**
  ...
  - When clicked on the header, the values in that column are sorted. 
  - This interactive table is ideal to see what tradable coins are on the market,
  - Algorithms have beeb used and can be easily sorted by ascending or descending order.
